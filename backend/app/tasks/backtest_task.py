...
def run_backtest_task(
    ...
    # Check if we're already inside an event loop (FastAPI context)
    try:
    ...
    if loop is None:
        ...
        return asyncio.run(
            _run_backtest_async(symbol, timeframe, n, days, params, user_id)
        )
    else:
        # Already inside an event loop (FastAPI) — use sync path
        return _run_backtest_sync(symbol, timeframe, n, days, params, user_id)
...
async def _run_backtest_async(
    ...
    """Async backtest: load candles + run engine + persist in a single event loop."""
    import math
    ...
    candles = await load_candles(symbol, timeframe, n)
...
def _run_backtest_sync(
    ...
    """Sync backtest fallback for when called from within an event loop (FastAPI)."""
    import math
