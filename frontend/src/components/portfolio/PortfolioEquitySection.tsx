...
export function PortfolioEquitySection() {
  ...
  if (isLoading) {
    return (
      <div className="bg-(--color-bg-surface) border border-(--color-border) rounded-xl p-5 flex items-center justify-center min-h-[380px]" role="status" aria-live="polite">
        <Loader2 className="w-6 h-6 animate-spin text-(--color-accent)" aria-hidden="true" />
        <span className="sr-only">Loading equity data</span>
      </div>
    );
  }
  ...
  if (!data || data.points.length === 0) {
    return (
      <div className="bg-(--color-bg-surface) border border-(--color-border) rounded-xl p-5 flex flex-col items-center justify-center min-h-[380px] gap-3">
        <Activity className="w-10 h-10 text-(--color-text-secondary)/40" aria-hidden="true" />
      ...
      <div className="grid grid-cols-3 gap-1.5 sm:gap-3">
        <div className="bg-(--color-bg-elevated)/50 rounded-lg p-2 sm:p-3 space-y-0.5">
          <div className="flex items-center gap-1 sm:gap-1.5">
            <TrendingUp className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-(--color-text-secondary)" aria-hidden="true" />
            <span className="text-xs text-(--color-text-secondary) uppercase tracking-wider">Return</span>
          </div>
          <p className={cn("text-sm sm:text-base font-semibold font-mono truncate", pnlColor(data.total_return_pct))}>
            {data.total_return_pct >= 0 ? "+" : ""}{data.total_return_pct.toFixed(2)}%
          </p>
        </div>
        <div className="bg-(--color-bg-elevated)/50 rounded-lg p-2 sm:p-3 space-y-0.5">
          <div className="flex items-center gap-1 sm:gap-1.5">
            <ArrowDown className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-(--color-text-secondary)" aria-hidden="true" />
            <span className="text-xs text-(--color-text-secondary) uppercase tracking-wider">Max DD</span>
          </div>
          <p className="text-sm sm:text-base font-semibold font-mono text-(--color-negative) truncate">
            {data.max_drawdown_pct.toFixed(2)}%
          </p>
        </div>
        <div className="bg-(--color-bg-elevated)/50 rounded-lg p-2 sm:p-3 space-y-0.5">
          <div className="flex items-center gap-1 sm:gap-1.5">
            <BarChart3 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-(--color-text-secondary)" aria-hidden="true" />
            <span className="text-xs text-(--color-text-secondary) uppercase tracking-wider">Sharpe</span>
          </div>
          <p className={cn("text-sm sm:text-base font-semibold font-mono truncate", pnlColor(data.sharpe_ratio))}>
