...
import { useStrategy, useToggleStrategy, useDeleteStrategy, useUpdateStrategy } from "@/hooks/useStrategies";
...
type Tab = "signals" | "config";

const TABS: { key: Tab; label: string; icon: typeof Zap; tip: string }[] = [
  { key: "signals", label: "Signals", icon: Zap, tip: "View all signals generated by this strategy and their execution status." },
  { key: "config", label: "Config", icon: Settings2, tip: "Configure exchange routing per trading pair." },
];
...
/* ----- Signal Status Badge with Reasoning Tooltip (hover) ----- */

function SignalStatusWithTooltip({
  signal,
  variant,
}: {
  signal: Signal;
...
export function StrategyDetailPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 max-w-[1600px] mx-auto">
              ...
              <table className="w-full text-left text-sm min-w-[1000px]">
                ...
                <tbody className="divide-y divide-(--color-border)/50">
                  {signalsLoading ? (
                    ...
                    <tr>
                      <td colSpan={10} className="px-3 py-16 text-center text-(--color-text-secondary)">
                        {debouncedSearch || dirFilter || statusFilter
      ...
      {activeTab === "config" && (
        <ExchangeMapEditor strategyId={strategy.id} config={config} />
      )}
...
function SignalRow({ signal }: { signal: Signal }) {
  ...
  return (
    <tr className="hover:bg-(--color-bg-elevated)/50 transition-colors">
      ...
      <td className="px-3 py-2.5">
        <SignalStatusWithTooltip signal={signal} variant={statusVariant} />
      </td>
    </tr>
  );
}
...
function ExchangeMapEditor({
  ...
  // Init local state from saved config
  useEffect(() => {
  ...
  return (
    <div className="bg-(--color-bg-surface) border border-(--color-border) rounded-xl p-5 space-y-5 max-w-2xl">
      <div>
        <h3 className="text-sm font-semibold text-(--color-text-primary)">
          Exchange Routing
        </h3>
        <p className="text-xs text-(--color-text-secondary) mt-1">
          Choose which exchange to use for each trading pair. This controls where candle data
          is fetched from{hasLiveTrading ? " and where orders are executed" : ""}.
        </p>
        {!hasLiveTrading && (
          <p className="text-xs text-(--color-warning) mt-1.5 flex items-center gap-1.5">
        ...
        <div className="space-y-3">
          {symbols.map((sym) => (
            <div key={sym} className="flex items-center gap-4">
              <span className="text-sm font-semibold text-(--color-accent) bg-(--color-accent-soft) rounded-md px-3 py-1.5 min-w-[120px] text-center">
                {sym}
              </span>
              <select
                value={exchangeMap[sym] ?? "binance"}
                onChange={(e) =>
                  setExchangeMap((prev) => ({ ...prev, [sym]: e.target.value }))
                }
                className="flex-1 h-9 rounded-lg border border-(--color-border) bg-(--color-bg-elevated) px-3 text-sm text-(--color-text-primary) focus:outline-none focus:ring-1 focus:ring-(--color-accent) appearance-none cursor-pointer"
              >
